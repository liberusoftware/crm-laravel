...
final class AddStep
{
    public function execute(int $teamId, int $actorId, array $data): EngagementStep
    {
        ...
        $data = validator($data, ['sequence_id' => ['required', 'integer'], 'position' => ['required', 'integer', 'min:1'], 'channel' => ['required', 'in:email,sms,call,social,task'], 'delay_minutes' => ['nullable', 'integer', 'min:0'], 'template' => ['nullable', 'string'], 'snippet' => ['nullable', 'array']])->validate();
